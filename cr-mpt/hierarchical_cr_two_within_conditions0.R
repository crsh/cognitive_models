...
# with random participant intercept and homogeneous
...
model {
  ...
  for(c1 in 1:n_c1) {
    for(c2 in 1:n_c2) { # V = xi_part[1:(n_c1 + n_c2)]; G = xi_part[(n_c1 + n_c2 + 1):(n_param - 1)]
      V_hat[1:n_subject, c1, c2] <- mu_V_hat[c1, c2] + xi_part[1] * delta_mu_hat_part[1:n_subject, 1]
      G_hat[1:n_subject, c1, c2] <- mu_G_hat[c1, c2] + xi_part[2] * delta_mu_hat_part[1:n_subject, 2]
    }
  }
  b_hat[1:n_subject] <- mu_b_hat + xi_part[n_param] * delta_mu_hat_part[1:n_subject, n_param]
