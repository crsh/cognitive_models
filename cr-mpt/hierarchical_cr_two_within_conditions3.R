...
# with random participant intercept and slope, heterogeneous
...
data {
  ...
  n_param <- 2 * n_c1 * n_c2 + 1 # V & G for each condition, common b
...
model {
  ...
  for(c1 in 1:n_c1) {
    for(c2 in 1:n_c2) {
      V_hat[1:n_subject, c1, c2] <- mu_V_hat[c1, c2] + xi_part_V[c1, c2] * delta_mu_hat_part[1:n_subject, (2*c2-2)^2 + c1]
      G_hat[1:n_subject, c1, c2] <- mu_G_hat[c1, c2] + xi_part_G[c1, c2] * delta_mu_hat_part[1:n_subject, n_c1 + (2*c2-2)^2 + c1]
    }
  }
