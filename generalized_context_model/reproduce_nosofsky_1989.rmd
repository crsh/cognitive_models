# Reproduction of model-based analyses by Nosofsky (1989)
To validate this implementation of the Generalized Context Model, I reproduced small parts of the model-based analyses reported in Nosofsky (1989). The original MDS solutions and response data were provided by Robert Nosofsky (s. [Note on data](data/README.html)).
...
size_pred <- gcm_pred(size_fit$par, mem[, 1:3], obs, rho = 2)
```
...
angle_pred <- gcm_pred(angle_fit$par, mem[, c(1:2, 4)], obs, rho = 2)
```
