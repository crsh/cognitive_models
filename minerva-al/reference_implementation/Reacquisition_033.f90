...
implicit none
	...
	Memory = 0.0	  
		...
		do k = 1, N_trials
			...
			if (j == 3) Probe(:) = Cue_matrix(:, 1) + Cue_matrix(:, 9) + Cue_matrix(:, 10)  ! Reacquisition
			if (j == 4) Probe(:) = Cue_matrix(:, 1) + Cue_matrix(:, 9) + Cue_matrix(:, 10)  ! Reacquisition
			if (j == 5) Probe(:) = Cue_matrix(:, 1) + Cue_matrix(:, 9) + Cue_matrix(:, 10)  ! Reacquisition
	...
	!---------------------------------------------------
	Open(1, file='results/Reacquisition_033.txt')
	write(1,*)
