...
for (r in 1:n_replications) {
  for(i in 1:3) {
    ...
    normalized_echo <- get_normalized_echo(probe, NULL, cue_features)
    expectancy <- get_expectancy(outcome, normalized_echo)
    memory <- learn(
      normalized_echo
      , acquisition_event
      , p_encode[i]
    ...
    for(j in 2:(n_trials / 2)) {
      normalized_echo <- get_normalized_echo(probe, memory, cue_features)
      expectancy <- c(expectancy, get_expectancy(outcome, normalized_echo))
      memory <- learn(
        normalized_echo
        , acquisition_event
        , p_encode[i]
