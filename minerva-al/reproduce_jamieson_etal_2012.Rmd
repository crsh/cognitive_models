...
for(i in 1:length(p_encode)) {
  lines(colMeans(reacquisition_sim_results[(1:n_replications) + (i-1)*n_replications, 1:50]), lwd = 2)
}

for(i in 1:length(p_encode)) {
  lines(colMeans(reacquisition_sim_results[(1:n_replications) + (i-1)*n_replications, 101:150]), lty = 2)
}
```
